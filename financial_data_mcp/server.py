...
from .fisis_client import LARGE_DIVISIONS, FISIS_SECTOR_CODES, FISIS_CODE_TO_SECTOR, FISIS_LARGE_GROUPS, FisisClient
...
_DATA_CATALOG = {
    ...
    "FISIS": {
        ...
        "lrg_div": {
            "은행":    {"A": "국내은행", "J": "외은지점"},
            "보험":    {"H": "생명보험", "I": "손해보험"},
            "금융투자": {"F": "투자매매중개업자I(증권)", "W": "선물사", "G": "집합투자업자(자산운용)", "X": "투자자문일임업자", "D": "종합금융회사", "M": "부동산신탁"},
            "비은행":  {"C": "신용카드사", "K": "리스사", "T": "할부금융사", "N": "신기술금융사", "E": "상호저축은행", "O": "신용협동조합", "Q": "농업협동조합", "P": "수산업협동조합", "S": "산림조합"},
            "기타":    {"L": "금융지주회사", "B": "공통(신탁)", "R": "공통(파생상품)"},
        },
        "strength": [
        ...
        "common_stat_codes": {
            ...
            "신용카드사(lrg_div=C)": {
                "_note": "신용카드사 전용 통계(SC 코드). 리스사=lrg_div=K(SK코드), 할부금융사=lrg_div=T(ST코드), 신기술금융사=lrg_div=N(SN코드) 각각 별도 조회.",
                "_재무현황": {
            ...
            "종합금융회사(lrg_div=D)": {
                "_note": "종합금융회사 통계 (SD 코드). 증권사는 lrg_div=F(SF 코드) 사용.",
                "_재무현황": {
                    "SD103": "요약재무상태표(자산)(11.06이후)",
                    "SD104": "요약재무상태표(부채 및 자본)(11.06이후)",
                    "SD107": "요약손익계산서(11.06이후) — 종합금융회사",
                    "SD012": "부문별 손익(수수료)",
    ...
    "key_stat_codes_by_sector": {
        ...
        "신용카드사(lrg_div=C)": {
            "_note": "전업카드사(삼성·현대·신한·KB·롯데카드 등) 전용. 캐피탈사=K, 할부=T",
        ...
        "종합금융회사(lrg_div=D)": {
            "_note": "증권사 통계는 lrg_div=F (SF코드) 사용",
            "수익성_ROA_ROE": "SD010",
            "여신건전성": "SD009",
            "자본적정성": "SD008",
        },
    ...
    "fisis_registration_status": {
        ...
        "등록_업권": [
            "국내은행 (lrg_div=A)", "외은지점 (lrg_div=J)",
            "생명보험 (lrg_div=H)", "손해보험 (lrg_div=I)",
            "신용카드사 (lrg_div=C)", "리스사 (lrg_div=K)", "할부금융사 (lrg_div=T)", "신기술금융사 (lrg_div=N)",
            "투자매매중개업자I-증권사 (lrg_div=F)", "선물사 (lrg_div=W)",
            "집합투자업자-자산운용사 (lrg_div=G)", "투자자문·일임업자 (lrg_div=X)",
            "종합금융회사 (lrg_div=D)", "부동산신탁 (lrg_div=M)", "상호저축은행 (lrg_div=E)",
            "금융지주회사 (lrg_div=L)",
            "신용협동조합 (lrg_div=O)", "농업협동조합 (lrg_div=Q)", "수산업협동조합 (lrg_div=P)", "산림조합 (lrg_div=S)",
            "공통(신탁) (lrg_div=B)", "공통(파생상품) (lrg_div=R)",
        ],
        "미등록_DART_단독": [
            "캐피탈사는 FISIS에서 lrg_div=K(리스사) 통계로 조회되나 개별사 finance_cd 미등록 다수 — 개별사 조회 불가 시 DART 단독",
            "신용협동조합(O)·농협(Q)·수협(P)·산림조합(S) — 통계는 있으나 개별사 식별 제한",
            "새마을금고 — FISIS 미등록, DART 단독 전략",
        ],
        "rule": "개별사 finance_cd 미확인 업권은 fisis_list_companies로 먼저 확인 후 조회. 미등록 확인 시 DART 재무제표 단독 전략 전환하고 data_gaps에 명시.",
    },
}
...
async def fisis_list_statistics(
    ...
    Args:
        lrg_div: 금융권역 코드 — 반드시 지정해야 합니다 (비워두면 API 오류 반환).
            A=국내은행, J=외은지점,
            H=생명보험, I=손해보험,
            F=증권사(투자매매I), W=투자매매II, G=자산운용사, X=투자자문, D=종합금융, M=부동산신탁,
            C=신용카드사(SC코드 — 할부금융·리스 부문 포함), K=리스사, T=할부금융사, N=신기술금융사,
            E=저축은행, O=신협, Q=농협, P=수협, S=산림조합,
            L=금융지주, B=공통(신탁), R=공통(파생상품)
        sml_div: 소분류 (비워두면 전체)
    """
...
async def fisis_get_statistics(
    ...
    **단일 코드 1회 조회 전용. 복수 코드 조회 시 반드시 fisis_get_multi_statistics를 사용하세요.**
    ...
    Args:
        ...
        lrg_div: 금융권역 코드 (A=국내은행, J=외은지점, H=생명보험, I=손해보험, F=증권사, W=투자매매II, G=자산운용사, X=투자자문, D=종합금융, M=부동산신탁, C=신용카드사, K=리스사, T=할부금융사, N=신기술금융사, E=저축은행, O=신용협동조합, Q=농협, P=수협, S=산림조합, L=금융지주, B=공통신탁, R=공통파생)
        sml_div: 소분류 코드
...
async def fisis_list_companies(
    ...
    Args:
        lrg_div: 금융권역 코드 (A=국내은행, J=외은지점, H=생명보험, I=손해보험, F=증권사, G=자산운용사, C=신용카드사, K=리스사, T=할부금융사, N=신기술금융사, E=저축은행, O=신용협동조합, L=금융지주 등)
        sml_div: 소분류
        finance_cd: 금융회사코드 (특정 회사만 조회 시, fisis_list_companies로 확인)
    """
...
# DART 업종코드 → FISIS 금융권역 매핑 (금감원 금융권역 분류표 기반)
# (lrg_div 코드, 업권명) 튜플
_INDUTY_TO_FISIS_DIV: dict[str, tuple[str, str]] = {
    ...
    "종합금융": ("D", "종합금융회사"),
    # 보험 — 생명보험(H), 손해보험(I)
    "보험": ("H", "생명보험"),   # 키워드 폴백: 기본값 생명보험
    "651": ("H", "생명보험"),
    "6511": ("H", "생명보험"),
    "6512": ("I", "손해보험"),
    "생명보험": ("H", "생명보험"),
    "손해보험": ("I", "손해보험"),
    # 여신전문 — 카드사(C), 리스사(K), 할부금융사(T), 신기술금융사(N)
    "카드": ("C", "신용카드사"),
    "리스": ("K", "리스사"),
    "할부": ("T", "할부금융사"),
    "캐피탈": ("K", "리스사"),   # 캐피탈사 = 리스사 권역
    "신기술": ("N", "신기술금융사"),
    "649": ("C", "신용카드사"),  # 여신전문 통합 업종코드 → 카드사로 기본 매핑
    "6491": ("C", "신용카드사"),
    "6492": ("K", "리스사"),
    # 저축은행·상호금융
    "저축은행": ("E", "상호저축은행"),
